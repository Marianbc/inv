...
var lastItemSold = "";
console.log("inventory edited! v11");
var lockedSlots = [];
...
function reloadStorageData(invData)
{
	...
	if(userVars["DFSTATS_df_storage_slots"] >= 400)
	{
		if(document.getElementById("buyStorageSlots") && document.getElementById("buyStorageSlots").querySelector("span"))
		{
			...
			switch(dataBank["itemcat"])
			{
				case "weapon":
					...
					if(dataBank["type"])
					{
						...
						} else if(dataBank["type"] === "machinegun" || dataBank["type"] === "minigun" || dataBank["type"] === "bigmachinegun")
						{
...
function initiateCrafting()
{
	...
	craftCategory.style.left = "118px";
	craftCategory.style.right = "118px";
		...
		if(infoBox.style.visibility === "hidden" || curInfoItem !== target)
		{
			...
			} else if(itemData["itemcat"] === "item")
			{
				...
				} else
				{
					...
					if(parseInt(itemData["boostexphours"]) > 0)
					{
						if(parseInt(itemData["boostexphours"]) === 1)
						{
							icOut += "<div class='itemData'>Boosts exp gain by +50% for " + itemData["boostexphours"] + " hour</div>";
						} else
						{
							icOut += "<div class='itemData'>Boosts exp gain by +50% for " + itemData["boostexphours"] + " hours</div>";
						}
					}
					if(parseInt(itemData["boostdamagehours"]) > 0)
					{
						if(parseInt(itemData["boostdamagehours"]) === 1)
						{
							icOut += "<div class='itemData'>Boosts damage by +35% for " + itemData["boostdamagehours"] + " hour</div>";
						} else
						{
							icOut += "<div class='itemData'>Boosts damage by +35% for " + itemData["boostdamagehours"] + " hours</div>";
						}
					}
					if(parseInt(itemData["boostspeedhours"]) > 0)
					{
						if(parseInt(itemData["boostspeedhours"]) === 1)
						{
							icOut += "<div class='itemData'>Boosts run/walk speed by +35% for " + itemData["boostspeedhours"] + " hour</div>";
						} else
						{
							icOut += "<div class='itemData'>Boosts run/walk speed by +35% for " + itemData["boostspeedhours"] + " hours</div>";
						}
					}
...
function dragDropAction(e)
{
	...
	switch(nSlot.dataset.action)
	{
		...
		case "enhance":
			...
			{
				prompt.textContent += "There is a chance this item will receive worse stats. ";
			}
			prompt.textContent += "Are you sure you want to enhance the ";
			if(itemData[1].indexOf("_name") >= 0)
			{
				prompt.textContent += itemData[1].substring(itemData[1].indexOf("_name") + 5);
			} else
			{
				prompt.textContent += globalData[itemType[0]]["name"];
			}
			prompt.textContent += " for $" + nf.format(enhancePrice) + "?";
			action = enhanceItem;
			question = true;
			break;
		case "adye":
			...
			prompt.textContent = "This will give the item a random colour. Are you sure you want to re-colour the ";
			if(itemData[1].indexOf("_name") >= 0)
			{
				prompt.textContent += itemData[1].substring(itemData[1].indexOf("_name") + 5);
			} else
			...
			prompt.textContent += " for $" + nf.format(dyePrice) + "?";
			action = dyeItem;
			question = true;
			break;
		case "mdye":
			...
			//var servData = [itemData, [0, "", "discard"]];
			prompt.textContent = "Please choose a colour below for the ";
			if(itemData[1].indexOf("_name") >= 0)
			{
				prompt.textContent += itemData[1].substring(itemData[1].indexOf("_name") + 5);
			} else
			...
			prompt.textContent += ". This will cost ";
			switch(globalData[itemType[0]]["itemcat"])
			{
				case "armour":
					prompt.textContent += 250;
					extraData = [itemData, 250];
					break;
				case "item":
					prompt.textContent += 100;
					extraData = [itemData, 100];
					break;
			}
			prompt.textContent += " credits.";
		...
		case "godcraft":
			...
			//var servData = [itemData, [0, "", "discard"]];
			var enhancePrice = enhanceValue(itemType[0]);
			prompt.textContent = "Are you sure you want to godcraft the ";
			if(itemData[1].indexOf("_name") >= 0)
			{
				prompt.textContent += itemData[1].substring(itemData[1].indexOf("_name") + 5);
			} else
		...
		case "rename":
			...
			prompt.textContent = "Please pick an appropriate name for your ";
			if(itemData[1].indexOf("_name") >= 0)
			{
				prompt.textContent += itemData[1].substring(itemData[1].indexOf("_name") + 5);
			} else
			...
			prompt.textContent += ". This will cost 500 credits.";
		...
		case "giveToChar":
			...
			} else if(parseInt(globalData[itemType[0]]["boostdamagehours"]) > 0 || parseInt(globalData[itemType[0]]["boostexphours"]) > 0 || parseInt(globalData[itemType[0]]["boostspeedhours"]) > 0)
			{
				playSound("heal");
...
function nicknameConfirm(data)
{
	var nickname = prompt.querySelector("input").value;

	prompt.textContent = "Are you sure you want to rename your ";
	if(data[0][1].indexOf("_name") >= 0)
	{
		prompt.textContent += data[0][1].substring(data[0][1].indexOf("_name") + 5);
	} else
	{
		var itemType = data[0][1].split('_');
		prompt.textContent += globalData[itemType[0]]["name"];
	}
	prompt.textContent += " to " + nickname + " for 500 credits?";
			...
			} else if(slot.classList.contains("fakeSlot") || (slot.classList.contains("fakeItem") || slot.classList.contains("profitList")) && slot.parentNode.classList.contains("fakeSlot") || (slot.parentNode.classList.contains("fakeItem") || slot.parentNode.classList.contains("profitList")) && slot.parentNode.parentNode.classList.contains("fakeSlot"))
			{
				...
				switch(slot.dataset.action)
				{
					...
					case "giveToChar":
						...
						} else if(parseInt(globalData[itemType]["boostdamagehours"]) > 0 || parseInt(globalData[itemType]["boostexphours"]) > 0 || parseInt(globalData[itemType]["boostspeedhours"]) > 0)
						{
